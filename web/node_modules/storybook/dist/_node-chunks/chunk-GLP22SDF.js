...
function getChromaticVersionSpecifier(packageJson) {
  ...
  return dependency || (packageJson.scripts && Object.values(packageJson.scripts).find((s2) => s2?.match(/chromatic/)) ? "latest" : void 0);
}
...
var getLastEvents = async () => (await processingPromise, await cache3.get("lastEvents") || {}), upgradeFields = (event) => {
  ...
  let descendingEvents = eventTypes.map((eventType) => lastEvents?.[eventType]).filter((event) => !!event).sort((a2, b2) => b2.timestamp - a2.timestamp);
