...
import {
  toEstree
} from "../_node-chunks/chunk-RMXUOI4J.js";
import {
  StatusTypeIdMismatchError
} from "../_node-chunks/chunk-SXH5N44R.js";
import {
  debounce,
  generateStoryFile,
  getErrorLevel,
  ...
  useStatics,
  withTelemetry
} from "../_node-chunks/chunk-CRA4LABS.js";
import {
  detectPnp
} from "../_node-chunks/chunk-ZTT3ANLN.js";
import "../_node-chunks/chunk-CHU3PQLW.js";
import {
  StorybookError
} from "../_node-chunks/chunk-UWL6FCZO.js";
import {
  telemetry
} from "../_node-chunks/chunk-GLP22SDF.js";
import "../_node-chunks/chunk-Z7XDXOYY.js";
import {
...
} from "../_node-chunks/chunk-X6B7XGEZ.js";
import "../_node-chunks/chunk-B4NB3TKA.js";
import "../_node-chunks/chunk-UQIRZH3P.js";
import {
  logger,
  require_pretty_hrtime
} from "../_node-chunks/chunk-CHR7DWLX.js";
import "../_node-chunks/chunk-4MHPXRYQ.js";
import "../_node-chunks/chunk-ZTC4BWCE.js";
import {
  invariant,
  up2 as up
} from "../_node-chunks/chunk-PHLUWINB.js";
import "../_node-chunks/chunk-QQORASXM.js";
import {
  importModule,
  resolvePackageDir
} from "../_node-chunks/chunk-AX5YUU2A.js";
import {
  dirname,
  isAbsolute,
  join,
  relative,
  resolve
} from "../_node-chunks/chunk-HWHPSPDL.js";
import {
  slash
} from "../_node-chunks/chunk-6NEWBUYZ.js";
import "../_node-chunks/chunk-734OFJUC.js";
import {
  Tag
} from "../_node-chunks/chunk-DT5QWWL3.js";
import {
  require_dist
} from "../_node-chunks/chunk-CXC3OGVY.js";
import {
  require_lib,
  require_src
} from "../_node-chunks/chunk-U7SRW327.js";
import {
  require_picocolors
} from "../_node-chunks/chunk-PZCOES5L.js";
import {
  __commonJS,
  __require,
  __toESM
} from "../_node-chunks/chunk-PQJM45KS.js";
...
async function getManagerBuilder() {
  return await import("../_node-chunks/builder-manager-45GNGM35.js");
}
  ...
  async function onChangeOrRemove(absolutePath, removed) {
    ...
    !removed && isDirectory(absolutePath) && await Promise.all(
      specifiers.filter((specifier) => importPath.startsWith(specifier.directory)).map(async (specifier) => {
        ...
        ), { globby } = await import("../_node-chunks/globby-OF6XRIN4.js");
...
function registerIndexJsonRoute({
  ...
  let maybeInvalidate = debounce(() => channel.emit(STORY_INDEX_INVALIDATED), DEBOUNCE, {
    ...
    ${!a.hasPropTypeError && propEntries.length > 0 ? `
        <div class="panel panel-props">
          <div class="note ok">
            ...
            <pre><code>Component: ${reactDocgen?.definedInFile ? esc(path.relative(process.cwd(), reactDocgen.definedInFile)) : reactDocgenTypescriptData?.filePath ? esc(path.relative(process.cwd(), reactDocgenTypescriptData.filePath)) : ""}${reactDocgen?.exportName ? "::" + esc(reactDocgen.exportName) : reactDocgenTypescriptData?.exportName ? "::" + esc(reactDocgenTypescriptData.exportName) : ""}</code></pre>
...
var getAttr = (elt, what) => elt.attributes.find((n) => n.type === "JSXAttribute" && n.name.name === what), getAttrValue = (elt, what) => getAttr(elt, what)?.value, getAttrLiteral = (elt, what) => {
  ...
  let store = {
    ...
    metaTags: void 0,
    imports: void 0
  }, { compile } = await import("../_node-chunks/mdx-N42X6CFJ-DHSIMX3W.js");
  await compile(code, {
...
var makeAbsolute = (otherImport, normalizedPath, workingDir) => otherImport.startsWith(".") ? slash(resolve3(workingDir, normalizeStoryPath(join5(dirname2(normalizedPath), otherImport)))) : otherImport, StoryIndexGenerator = class _StoryIndexGenerator {
  ...
  static async findMatchingFiles(specifier, workingDir, ignoreWarnings = !1) {
    ...
    if (cached)
      return cached;
    let pathToSubIndex = {}, globCwd = slash(resolve3(workingDir, specifier.directory)), globPattern = specifier.files.startsWith("!") ? `./${specifier.files}` : specifier.files, { globby } = await import("../_node-chunks/globby-OF6XRIN4.js"), files = await globby(globPattern, {
      absolute: !0,
      cwd: globCwd,
...
async function buildStaticStandalone(options) {
  ...
  let channel = new Channel({}), presets = await loadAllPresets({
  ...
  ), features?.componentsManifest && effects.push(writeManifests(options.outputDir, presets))), core2?.disableProjectJson || effects.push(
...
import { deprecate, logger as logger17, prompt } from "storybook/internal/node-logger";
...
import { logger as logger8, pretty } from "storybook/internal/client-logger";
...
// src/channels/postmessage/index.ts
var { document: document2, location } = global3;
...
var { WebSocket } = global4;

// src/channels/index.ts
var { CHANNEL_OPTIONS, CONFIG_TYPE } = global5, channels_default = Channel2;
...
import { setImmediate as setImmediate2 } from "node:timers";
...
var mount = (fn) => fn instanceof Polka ? fn.attach : fn, Polka = class _Polka extends Trouter {
  constructor(opts = {}) {
    super(), this.parse = parse5, this.server = opts.server, this.handler = this.handler.bind(this), this.onError = opts.onError || onError, this.onNoMatch = opts.onNoMatch || this.onError.bind(null, { status: 404 }), this.attach = (req, res) => setImmediate2(this.handler, req, res);
  }
  use(base, ...fns) {
...
Note: BROWSER environment variable is set to "${browserEnv}". To disable browser opening, use BROWSER=none or the --ci flag.` : "";
      logger10.error(import_ts_dedent4.dedent`
...
async function storybookDevServer(options, server) {
  let core2 = await options.presets.apply("core"), app = build_default2({ server }), workingDir = process.cwd(), configDir = options.configDir, stories = await options.presets.apply("stories"), normalizedStories = normalizeStories(stories, {
    configDir,
    workingDir
  ...
  if (app.use(build_default({ level: 1 })), typeof options.extendServer == "function" && options.extendServer(server), app.use(
  ...
  ), app.use(getAccessControlMiddleware(core2?.crossOriginIsolated ?? !1)), app.use(getCachingMiddleware()), registerIndexJsonRoute({
    app,
    storyIndexGeneratorPromise,
    normalizedStories,
    ...
    workingDir,
    configDir
  }), (await getMiddleware(options.configDir))(app), await options.presets.apply("experimental_devServer", app), !core2?.builder)
  ...
  options.ignorePreview || (logger11.debug("Starting preview.."), previewResult = await previewBuilder.start({
    startTime: process.hrtime(),
    options,
    router: app,
    server,
    ...
    throw logger11.error("Failed to build the preview"), process.exitCode = 1, await managerBuilder?.bail().catch(), await previewBuilder?.bail().catch(), e;
  }));
  ...
  (await options.presets.apply("features"))?.componentsManifest && registerManifests({ app, presets: options.presets }), doTelemetry(app, core2, storyIndexGeneratorPromise, options);
...
import { Channel as Channel3, HEARTBEAT_INTERVAL as HEARTBEAT_INTERVAL2 } from "storybook/internal/channels";
...
`), socket.destroy();
      ...
      wss.on("message", (raw) => {
        let data = raw.toString(), event = typeof data == "string" && (0, import_telejson3.isJSON)(data) ? (0, import_telejson3.parse)(data, {}) : data;
        this.handler?.(event);
      });
    });
  ...
  send(event) {
    let data = (0, import_telejson3.stringify)(event, { maxDepth: 15 });
    Array.from(this.socket.clients).filter((c) => c.readyState === WebSocket2.OPEN).forEach((client) => client.send(data));
  }
};
...
import { CLI_COLORS, logger as logger12 } from "storybook/internal/node-logger";
...
var getServerPort = (port, { exactPort } = {}) => (0, import_detect_port.default)(port).then((freePort) => (freePort !== port && exactPort && process.exit(-1), freePort)).catch((error) => {
  logger13.error(error), process.exit(-1);
...
async function getServer(options) {
  ...
  options.sslCert || (logger14.error("Error: --ssl-cert is required with --https"), process.exit(-1)), options.sslKey || (logger14.error("Error: --ssl-key is required with --https"), process.exit(-1));
  let sslOptions = {
...
var checkPackageCompatibility = async (dependency, context) => {
  ...
  try {
    ...
    return skipErrors || logger15.log(
...
var warnOnIncompatibleAddons = async (currentStorybookVersion, packageManager) => {
  ...
  incompatiblePackagesMessage && logger16.warn(incompatiblePackagesMessage);
};
...
async function buildDevStandalone(options) {
  ...
  options.port = port, options.versionCheck = versionCheck, options.configType = "DEVELOPMENT", options.configDir = configDir, options.cacheKey = cacheKey, options.outputDir = outputDir, options.serverChannelUrl = getServerChannelUrl(port, options), options.localAddress = localAddress, options.networkAddress = networkAddress, options.pnp = await detectPnp(), options.pnp && deprecate(import_ts_dedent8.dedent`
    ...
    `);
  let config = await loadMainConfig2(options), { core: core2, framework } = config, corePresets = [], frameworkName = typeof framework == "string" ? framework : framework?.name;
  ...
  try {
    ...
    logger17.warn("Storybook failed to check addon compatibility"), logger17.debug(`${e instanceof Error ? e.stack : String(e)}`);
  }
  try {
  ...
  if (options.host === "0.0.0.0" && (!allowedHosts || allowedHosts !== !0 && allowedHosts.length === 0) && logger17.warn(import_ts_dedent8.dedent`
  ...
  if (resolvedPreviewBuilder.includes("builder-vite")) {
    ...
    let mainJsContent = await readFile4(mainJsPath, { encoding: "utf8" }), CJS_CONTENT_REGEX = /\bmodule\.exports\b|\bexports[.[]|\brequire\s*\(|\bObject\.(?:defineProperty|defineProperties|assign)\s*\(\s*exports\b/, strippedContent = stripCommentsAndStrings(mainJsContent);
  ...
  global6.FEATURES = features, await presets.apply("experimental_serverChannel", channel);
  let fullOptions = {
    ...options,
    presets,
    features,
  ...
  if (options.smokeTest) {
    ...
    logger17.log(problems.map((p) => p.stack).join(`
...
import { Channel as Channel4 } from "storybook/internal/channels";
import {
...
async function loadStorybook(options) {
  ...
  let channel = new Channel4({}), presets = await loadAllPresets3({
    corePresets,
    overridePresets: [
      import.meta.resolve("storybook/internal/core-server/presets/common-override-preset")
    ],
  ...
  let features = await presets.apply("features");
  return global7.FEATURES = features, {
    ...options,
    presets,
    features
  };
...
var buildIndex = async (options) => {
  ...
  logger18.info(`Writing index to ${options.outputFile}`), await writeFile4(options.outputFile, JSON.stringify(index));
};
